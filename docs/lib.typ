...
#let bubble(
  ...
  logo: image("zju_logo_blue.svg"),
  main-color: "003F88",
  alpha: 0%,
  color-words: (),
  body,
) = {
  // Allow both a single author string and multiple authors.
  let authors = if type(author) == str {
  ...
  let author-line = authors.join("  ")

  set document(
    author: if authors.len() == 0 { "" } else { authors },
    title: title,
  )

  // Save heading and body font families in variables.
  // Western text uses Libertinus Serif first; Chinese text falls back to SimSun.
  let body-font = ("Libertinus Serif", "SimSun",  "Songti SC")
  let title-font = body-font
  let code-font = ("DejaVu Sans Mono", "JetBrainsMonoNL NF", "LXGW WenKai Mono Screen")
  ...
  // Main body.
  set page(
    header: [#emph()[#title #h(1fr) #author-line]],
  )
